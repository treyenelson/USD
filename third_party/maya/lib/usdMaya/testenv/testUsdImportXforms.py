...
class testUsdImportXforms(unittest.TestCase):
    ...
    def testImportMayaXformVariations(self):
        ...
        for i, enabledArray in enumerate(itertools.product((False, True), repeat=len(ATTRS))):
            ...
            for enabled, (attr, (valMin, valMax)) in zip(enabledArray,
                                                                    iter(ATTRS.items())):
                if not enabled:
        ...
        for node, attrVals in allExpected.items():
            ...
            for attr, expectedVal in attrVals.items():
    ...
    def testImportXformsRotateAxis(self):
        ...
        for rotOrderName, expectedRotation in expectedRotates.items():
